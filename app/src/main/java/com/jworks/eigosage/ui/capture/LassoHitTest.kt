...
/**
 * Convert a screen-space selection rectangle back to image-space coordinates.
 */
fun screenRectToImageRect(
    ...
    val left = ((minOf(start.x, end.x) - imageLeft) / scale).toInt().coerceAtLeast(0)
    val top = ((minOf(start.y, end.y) - imageTop) / scale).toInt().coerceAtLeast(0)
    val right = ((maxOf(start.x, end.x) - imageLeft) / scale).toInt().coerceAtMost(bitmap.width)
    val bottom = ((maxOf(start.y, end.y) - imageTop) / scale).toInt().coerceAtMost(bitmap.height)
